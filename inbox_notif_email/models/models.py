...
class MailThread(models.AbstractModel):
    ...
    #         default_followers = \
    #         self.env['mail.followers']._add_default_followers(self._name, [], self.env.user.partner_id.ids,
    #                                                           customer_ids=[])[0][0]
    ...
    def write(self, values):
        ...
        if tracked_fields:
            tracking = track_self.with_context(clean_context(self._context)).message_track(tracked_fields,
                                                                                           initial_values)
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(MailThread, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                      submenu=submenu)
        if view_type == 'form':
    ...
    def _routing_handle_bounce(self, email_message, message_dict):
        ...
        if bounced_email:
            ...
            if bounced_record and not bounced_record_done and issubclass(type(bounced_record),
        ...
        if bounced_record:
            _logger.info(
                ...
                bounced_msg_id, bounced_model, bounced_res_id)
        elif bounced_email:
            _logger.info(
       ...
       Only allowed people can contact us at this address.</p>
        ...
        if thread_id:
            if not record_set.exists():
                self._routing_warn(
                    _('reply to missing document (%s,%s), fall back on document creation') % (model, thread_id),
        ...
        if alias:
            ...
            if hasattr(obj, '_alias_check_contact'):
                ...
                check_result = self.env['mail.alias.mixin']._alias_check_contact_on_record(obj, message, message_dict,
                                                                                           alias)
    ...
    def message_parse(self, message, save_original=False):
        ...
        partner_ids = [x.id for x in
                       self._mail_find_partner_from_emails(tools.email_split(msg_dict['recipients']), records=self) if
                       x]
    ...
    def _mail_find_user_for_gateway(self, email, alias=None):
        ...
        if normalized_email and catchall_domain:
            left_part = normalized_email.split('@')[0] if normalized_email.split('@')[
                                                              1] == catchall_domain.lower() else False
            if left_part:
    ...
    def _mail_find_partner_from_emails(self, emails, records=None, force_create=False, extra_domain=False):
        ...
        if catchall_domain:
            domain_left_parts = [email.split('@')[0] for email in normalized_emails if
                                 email and email.split('@')[1] == catchall_domain.lower()]
            if domain_left_parts:
                found_alias_names = self.env['mail.alias'].sudo().search(
                    [('alias_name', 'in', domain_left_parts)]).mapped('alias_name')
                normalized_emails = [email for email in normalized_emails if
                                     email.split('@')[0] not in found_alias_names]
        ...
        if email_cids:
            ...
            for partner in new_pids:
                ...
                recipient_data['partners'].append(
                    {'id': partner.id, 'share': True, 'active': True, 'notif': 'email', 'type': 'channel_email',
                     'groups': []})
